  ...
  return (
    <Card className="overflow-hidden transition-all hover:shadow-md">
      <CardHeader className="p-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 truncate">
            <Link href={`/framework/${framework.id}`} className="font-medium hover:underline">
              {framework.name}
            </Link>
          </div>
        </div>
      ...
      <CardContent className="p-4 pt-0">
        <p className="line-clamp-2 text-sm text-muted-foreground">{framework.description}</p>
      ...
      <CardFooter className="flex items-center justify-between border-t bg-muted/50 p-4">
        ...
        <Button size="sm" variant="ghost" asChild>
          <a href={framework.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1">
            <span>Visit</span>
